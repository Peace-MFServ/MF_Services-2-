...
export const HARDWARE_GROUPS = [
  ...
  { id: "handlePassiveOutside", label: "Handle — passive leaf, outside", needs: "lock" },
  { id: "flushBolt", label: "Flush bolt", needs: "lock" },
  { id: "electricStrike", label: "Electric strike", needs: "lock" },
  { id: "doorCloser", label: "Door closer" },
  { id: "glazing", label: "Glazing" },
  { id: "ventilationGrill", label: "Ventilation grill" },
  { id: "dropSeal", label: "Automatic drop-down seal" },
  { id: "threshold", label: "Threshold" },
  { id: "dripCap", label: "Drip cap" },
...
export function hardwareGroupsFor(config = {}, resolution = resolveSteelDoor(config)) {
  ...
  const add = (id, options) => {
    ...
    if (!list.length && !(meta.needs && !lock)) return;
    groups.push({ ...meta, options: list.map(String) });
  };
