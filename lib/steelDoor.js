 ...
 *  One house rule sits on top of the manufacturer's list: a fire
 *  rated doorset has to be plastered, so the unfilled block frames —
 *  Block, Block thermal and Block small — are not offered on rated
 *  types; their plaster-board-filled twins stand in. (MF Services'
 *  call, September 2026; the DFM sheet itself lists them all.) */
const UNPLASTERED_BLOCKS = new Set(["block", "block-thermal", "block-small"]);

export function framesFor(type) {
  if (!type) return [];
  return type.frames
    .filter(id => !(type.minutes > 0 && UNPLASTERED_BLOCKS.has(id)))
    .map(frameById)
    .filter(Boolean);
}
