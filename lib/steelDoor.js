 ...
 *  rated doorset has to be plastered, so the unfilled Block small is
 *  not offered on rated types — the plaster-filled one stands in.
 *  (MF Services' call, September 2026; the DFM sheet itself lists
 *  both.) */
export function framesFor(type) {
  if (!type) return [];
  return type.frames
    .filter(id => !(type.minutes > 0 && id === "block-small"))
    .map(frameById)
    .filter(Boolean);
}
