...
export async function generateSteelDoorPDF({ config, projectData = {}, specType, resolution }) {
  ...
  // A doorset with its full ironmongery runs to twenty-odd rows. The
  // rows tighten to keep the sheet to one page rather than pushing the
  // standards onto a second one that carries nothing else.
  const TABLE_BOTTOM = 208;
  const roomPerRow = rows.length ? (TABLE_BOTTOM - ry) / rows.length : 7;
  const baseRowH = Math.max(5.6, Math.min(7, roomPerRow));
  const pad = (baseRowH - 3.8) / 2;
  ...
  rows.forEach((r, i) => {
    ...
    const rowH = Math.max(baseRowH, valueLines.length * 3.8 + 2 * pad);

    // A fully specified doorset carries a lot of ironmongery — the
    // table runs on rather than off the bottom of the sheet.
    if (ry + rowH > pageH - 20) {
      doc.addPage();
      tableBroke = true;
      ry = margin;
      doc.setFont("helvetica", "bold"); doc.setFontSize(7.5); ink(doc, MUTED);
      doc.text("SPECIFICATION, CONTINUED", rightX, ry);
      ry += 4.5;
    }

    box(doc, rightX, ry, rightW, rowH, i % 2 === 0 ? "#FFFFFF" : SUNKEN);
    const textY = ry + pad + 2.8;
    doc.setFont("helvetica", "normal"); doc.setFontSize(8); ink(doc, BODY);
    doc.text(r.label, rightX + 2.5, textY);
    doc.setFont("helvetica", "bold"); doc.setFontSize(8); ink(doc, INK);
    valueLines.forEach((l, li) => doc.text(l, rightX + rightW - 2.5, textY + li * 3.8, { align: "right" }));
  ...
  // Once the table has moved onto a sheet of its own, the elevation is
  // no longer beside it — carry on beneath the table.
  y = (tableBroke ? ry : Math.max(leftY, ry)) + 8;
