...
describe("the specification sheet", () => {
  ...
  it("states how many of this doorset, one unless told otherwise", () => {
    expect(steelSpecRows(complete)[0]).toEqual({ label: "Quantity", value: "1" })
    expect(steelSpecRows({ ...complete, quantity: "6" })[0].value).toBe("6")
  })
...
describe("the plastering rule on fire rated doorsets", () => {
  // Block small only exists on the High Performance range, so the
  // rule is exercised there.
  it("keeps the unfilled Block small off rated types", () => {
    const rated = resolveSteelDoor({ minutes: 60, leaves: 1, highPerformance: true });
    const ids = rated.frames.map(f => f.id);
    expect(ids).not.toContain("block-small");
    expect(ids).toContain("block-small-filled-with-plaster-board");
  });

  it("still offers it on unrated doorsets", () => {
    const unrated = resolveSteelDoor({ minutes: 0, leaves: 1, highPerformance: true });
    expect(unrated.frames.map(f => f.id)).toContain("block-small");
  });

  it("keeps the unfilled Block and Block thermal off rated types too", () => {
    const rated = resolveSteelDoor({ minutes: 60, leaves: 2, highPerformance: false });
    const ids = rated.frames.map(f => f.id);
    expect(ids).not.toContain("block");
    expect(ids).not.toContain("block-thermal");
    expect(ids).toContain("block-with-plaster-board");
    expect(ids).toContain("block-thermal-with-plaster-board");
  });

  it("still offers the plain blocks on unrated doorsets", () => {
    const unrated = resolveSteelDoor({ minutes: 0, leaves: 1, highPerformance: false });
    const ids = unrated.frames.map(f => f.id);
    expect(ids).toContain("block");
    expect(ids).toContain("block-thermal");
  });
});
