...
describe("ETS 73 double-leaf sheet fidelity", () => {
  ...
  it("draws the Flatscan as one scanner per leaf and sensor strips on both leaves", () => {
    const flatscan = double.drawing.anchors["comp-10"]
    expect(flatscan.devices).toHaveLength(2)
    expect(flatscan.devices.every(d => d.kind === "flatscan")).toBe(true)

    const strips = double.drawing.anchors["comp-11"]
    expect(strips.devices).toHaveLength(2)
    expect(strips.devices.every(d => d.kind === "strip")).toBe(true)
  })
