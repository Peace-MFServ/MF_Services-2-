...
describe("validation", () => {
  ...
  it("rejects dimensions outside the stated limits", () => {
    const wild = validateSpec(product, config({ width: "1000000000", height: "1000000" }), {})
    expect(wild.errors.some(e => e.field === "width")).toBe(true)
    expect(wild.errors.some(e => e.field === "height")).toBe(true)

    const tiny = validateSpec(product, config({ width: "50", height: "50" }), {})
    expect(tiny.errors.some(e => e.field === "width")).toBe(true)
    expect(tiny.errors.some(e => e.field === "height")).toBe(true)
  })

  it("accepts the largest approvable opening", () => {
    const v = validateSpec(product, config({ width: "4347", height: "2639", leaves: 6 }), {})
    expect(v.errors.some(e => e.field === "width" || e.field === "height")).toBe(false)
  })
...
describe("quantity on the sheet", () => {
  it("prints first, defaulting to one of this doorset", () => {
    const res = resolveProduct(product, config())
    expect(specRows(product, config(), res)[0]).toEqual({ label: "Quantity", value: "1" })
    expect(specRows(product, config({ quantity: "6" }), res)[0].value).toBe("6")
    // A cleared field is still a doorset — one of it, not zero.
    expect(specRows(product, config({ quantity: "" }), res)[0].value).toBe("1")
  })
})
