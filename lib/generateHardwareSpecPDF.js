...
import { specRows } from "./hardwareSpec";
...
function drawElevation(doc, config, x, y, boxW, boxH) {
  ...
  for (let i = 0; i < leaves; i++) {
    ...
    if (leafW > 7 && leafH > 12) {
      box(doc, lx + leafW - 3, y0 + frameT + leafH / 2 - 2, 1.4, 4, "#8E9BA8", EDGE, 0.2);
    }
  }
...
export async function generateHardwareSpecPDF({ product, config, projectData, specType, resolution }) {
  ...
  drawElevation(doc, config, margin, bodyTop, leftW, 96);
  ...
  rows.forEach((r, i) => {
    box(doc, rightX, ry, rightW, 9, i % 2 === 0 ? "#FFFFFF" : SUNKEN);
    ...
    doc.text(doc.splitTextToSize(r.value, rightW - 48)[0], rightX + rightW - 2.5, ry + 5.8, { align: "right" });
    ry += 9;
  ...
  if (resolution?.status === "matched") {
    box(doc, margin, y, cW, 16, SUNKEN);
    box(doc, margin, y, 1.6, 16, NAVY);
    doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); ink(doc, MUTED);
    doc.text("PRODUCT", margin + 5, y + 6);
    doc.setFont("helvetica", "bold"); doc.setFontSize(11); ink(doc, INK);
    doc.text(resolution.product.name, margin + 5, y + 12);
    doc.setFont("helvetica", "normal"); doc.setFontSize(10); ink(doc, BODY);
    doc.text(resolution.product.code, pageW - margin - 5, y + 12, { align: "right" });
    y += 24;
  }
  ...
  y += 5;
  for (const s of product.standards) {
    if (y > pageH - 34) { doc.addPage(); y = margin; }
    doc.setFont("helvetica", "bold"); doc.setFontSize(9); ink(doc, INK);
    doc.text(s.code, margin, y + 4);
    doc.setFont("helvetica", "normal"); doc.setFontSize(8.5); ink(doc, BODY);
    doc.text(s.description, margin + 26, y + 4);
    y += 7;
    stroke(doc, RULE); doc.setLineWidth(0.12);
    doc.line(margin, y, pageW - margin, y);
    y += 1.5;
  ...
  if (branded && projectData.contactDetails?.trim()) {
    y += 6;
    doc.setFont("helvetica", "bold"); doc.setFontSize(7.5); ink(doc, MUTED);
    doc.text("ENQUIRY CONTACT", margin, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(9); ink(doc, BODY);
    doc.splitTextToSize(projectData.contactDetails.trim(), cW).slice(0, 3)
