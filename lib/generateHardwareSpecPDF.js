...
function drawElevation(doc, config, resolution, x, y, boxW, boxH) {
  ...
  // Architrave only when a visible frame style is chosen — the flush
  // standard reads as part of the wall.
  if (config.frameStyle && config.frameStyle !== "flush") {
    stroke(doc, EDGE); doc.setLineWidth(config.frameStyle === "raised-picture" ? 0.8 : 0.5);
    doc.rect(x0 - 1.8, y0 - 1.8, drawW + 3.6, drawH + 3.6, "S");
  }
  ...
  for (let i = 0; i < leaves; i++) {
    ...
    if (leafW > 7 && leafH > 12) {
      ...
      const pivotX = pivotsRight ? lx + leafW - 2.2 : lx + 0.5;
      for (const py of [y0 + frameT + 0.8, y0 + frameT + leafH - 2.6]) {
        box(doc, pivotX, py, 1.8, 1.8, "#6D7A88", EDGE, 0.2);
      }
      const isActive = leaves === 1 || (rightHand ? i === leaves - 1 : i === 0);
      if (isActive) {
        const lockX = pivotsRight ? lx + 1.6 : lx + leafW - 3;
        box(doc, lockX, y0 + frameT + leafH / 2 - 2, 1.4, 4, null, EDGE, 0.25);
      }
    }
  }
