...
import { specRows, isBespoke } from "./hardwareSpec";
...
export async function generateHardwareSpecPDF({ product, config, projectData, specType, resolution }) {
  ...
  // sources against it, so the sheet states what is being asked for.
  if (resolution?.status === "matched") {
    ...
    doc.text("BESPOKE ENQUIRY", margin + 5, y + 6);
