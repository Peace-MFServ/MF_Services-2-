...
function drawElevation(doc, config, resolution, x, y, boxW, boxH) {
  ...
  const leafSizeText = leafSize ? `${leafSize.width} x ${leafSize.height}` : null;
  ...
  for (const info of layout.leaves) {
    ...
    if (leafW > 7 && drawH > 12) {
      ...
      if (leafSizeText) {
        doc.setFont("helvetica", "normal"); doc.setFontSize(5.6); ink(doc, MUTED);
        if (leafW > 21) {
          doc.text(leafSizeText, lx + leafW / 2, y0 + drawH * 0.38, { align: "center" });
        } else {
          doc.text(leafSizeText, lx + leafW / 2 + 1.6, y0 + drawH * 0.38, { align: "center", angle: 90 });
        }
      }
    }
  }
