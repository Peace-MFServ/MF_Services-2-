...
export async function generateHardwareSpecPDF({ product, config, projectData, specType, resolution }) {
  ...
  let leftY = bodyTop + 90 + 5;

  // ── Basis of the size, in the space beneath the elevation ───
  ...
  const statusBox = (title, text, barHex, faceHex, titleHex) => {
    doc.setFont("helvetica", "normal"); doc.setFontSize(7);
    const lines = doc.splitTextToSize(text, leftW - 8);
    const h = 9.5 + lines.length * 3.1;
    box(doc, margin, leftY, leftW, h, faceHex);
    box(doc, margin, leftY, 1.6, h, barHex);
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); ink(doc, titleHex);
    doc.text(title, margin + 4.5, leftY + 5);
    doc.setFont("helvetica", "normal"); doc.setFontSize(7); ink(doc, BODY);
    lines.forEach((l, i) => doc.text(l, margin + 4.5, leftY + 9.5 + i * 3.1));
    leftY += h;
  };
  if (resolution?.status === "evidenced") {
    statusBox("COVERED BY TEST EVIDENCE", resolution.basis ?? "", NAVY, SUNKEN, NAVY);
  } else if (needsEvidence(resolution) || isBespoke(resolution)) {
    statusBox(
      isBespoke(resolution) ? "BESPOKE ENQUIRY" : "TEST REPORT TO CONFIRM",
      resolution.reason ?? "", WARN, "#FBF3EC", WARN,
    );
  }
  ...
  doc.setFont("helvetica", "bold"); doc.setFontSize(8);
  const codeW = Math.max(...product.standards.map(st => doc.getTextWidth(st.code))) + 6;
