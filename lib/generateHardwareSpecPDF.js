...
import { leafLayout } from "./doorLayout";
...
function drawElevation(doc, config, resolution, x, y, boxW, boxH) {
  ...
  // A multi-leaf riser is a row of door sets, each a pair meeting at a
  // centre mullion and pivoted at its outer edges. Christo doors are
  // pivot-hung — pivot pins, not hinges — and the active leaf of each
  // pair carries the lock.
  ...
  // Leaf size label. Every leaf is the same width, so the fit is worked
  // out once: horizontal always, shrunk to fit, and stacked onto two
  // lines rather than turned on its side.
  let leafLabel = null;
  if (leafSizeText) {
    doc.setFont("helvetica", "normal");
    const avail = leafW - 1.6;
    let size = 5.6;
    doc.setFontSize(size);
    while (size > 3.4 && doc.getTextWidth(leafSizeText) > avail) {
      size -= 0.3;
      doc.setFontSize(size);
    }
    leafLabel = doc.getTextWidth(leafSizeText) <= avail
      ? { size, lines: [leafSizeText] }
      : { size, lines: [String(leafSize.width), `× ${leafSize.height}`] };
  }

  for (const info of layout.leaves) {
    const lx = x0 + info.index * leafW;
    ...
    if (leafW > 7 && drawH > 12) {
      ...
      if (info.isActive) {
        ...
        // Passive leaf — the 2-point bolts, top and bottom.
        for (const by of [y0 + 4.5, y0 + drawH - 6.5]) {
          box(doc, lockX, by, 1.1, 1.8, "#6D7A88", EDGE, 0.2);
        }
      }
      ...
      if (leafLabel) {
        ...
        leafLabel.lines.forEach((line, n) => {
          doc.text(line, lx + leafW / 2, y0 + drawH * 0.38 + n * lineH, { align: "center" });
        });
      }
    }
  }
  ...
  for (const b of layout.boundaries) {
    box(doc, x0 + b * leafW - 0.6, y0, 1.2, drawH, "#8895A3", EDGE, 0.2);
  }
